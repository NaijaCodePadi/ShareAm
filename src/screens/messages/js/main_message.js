...
msgSample.forEach((item)=> {
    ...
    item.active ? chatContain.classList.add("active") : chatContain.classList.add("inactive");

    const imgDiv = document.createElement("div");
    imgDiv.classList.add("user-details-div1");
    imgDiv.innerHTML = `
        ${item.image ? `<img src="${item.image}" alt='User Profile Pic'/>`
            : `<div class="user-profile-pic-placeholder">NU</div>`}
    `

    chatContain.appendChild(imgDiv);

    const userDetails = document.createElement("div");
    userDetails.classList.add("user-details-div2");
    userDetails.innerHTML = `
        <div class="user-details">
            <section>
                <p class="${item.active ? `friends-display-name-text-active` : `friends-display-name-text`}">${item.name ? item.name : item.username}</p>
                ${item.verified ? `
                    <svg
    ...
    chatContain.appendChild(userDetails);
